...
#### TODOs ####
# TODO: Concatenate chromosome-level output to a single file

#### DESCRIPTION ####
# Peter Hickey (peter.hickey@gmail.com)
# 04/02/2014
# A example bash script for processing a BAM file with comethylation.py in a pseudo-parallel chromosome-by-chromsome fashion.

#### WARNINGS #####
# WARNING: This is not a robust, general purpose script. Rather it works for my particular setup and suggests how comethylation.py might be sped up by processing chromosome-level (or some other sub-genome level) units
...
# REQUIRES: samtools and Rscript must be in $PATH
...
#		(a) Process each (sorted) chromosome-level BAM with comethylation.py with that m-tuple value. Each chromosome is processed as a background job. 
...
COMETH= # The path to the script comethylation.py
...
COMETH_PARAMS= # Additional parameters to be passed to comethylation.py, e.g. '--methylationType CG --ignoreDuplicates --minQual 0 --ignoreStart_r1 5 --ignoreStart_r2 $3 --ignoreEnd_r1 10 --ignoreEnd_r2 3 --methylationType CG --overlappingPairedEndFilter XM'.
SEQ_TYPE= # "SE" (single-end data) or "PE" (paired-end data).

#### Step 0 ####
# Check parameters
if [[ -n "${BAM}" && -n "${OUTDIR}" && -n "${COMETH}" && -n "${SAMPLE_NAME}" && -n "${M}" && -n "${CHROMS}" && -n "${COMETH_PARAMS}" && -n "${SEQ_TYPE}" ]]
then
	...
	then
		...
		echo "Path to comethylation.py = ${COMETH}"
		echo "Sample name = ${SAMPLE_NAME}"
		echo "'m' in m-tuples = ${M[@]}"
		echo "Chromosomes = ${CHROMS[@]}"
		echo "Additional comethylation.py parameters = ${COMETH_PARAMS}"
...
do
	...
	for CHROM in ${CHROMS[@]}
	do      
		CMD="python ${COMETH} ${COMETH_PARAMS} --mTuple ${m} ${OUTDIR}/${SAMPLE_NAME}_${CHROM}.bam ${OUTDIR}/${m}_tuples/${SAMPLE_NAME}_${CHROM} &> ${OUTDIR}/${m}_tuples/log/${SAMPLE_NAME}_${CHROM}.${m}.log &"
		eval ${CMD}
	done
