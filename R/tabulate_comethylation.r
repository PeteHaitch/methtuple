...
library(multicore)
options(cores = 10) # Set the number of cores to be used by "multicore" package

## TODO: Add option to include arbitrary reference genome.
...
# x = a Bismark bam file
# reference = NA (not currently implemented)
...
##################################################################################################################################
# Functions for processing the XM "methylation string" tag in the Bismark SAM/BAM output
##################################################################################################################################
# A function for the Watson strand reads, the Crick strand reads and a wrapper function to process all reads
...
# x = a GRanges instance containing the reads as returned by readBismarkBam()
# cpg.pair = An IRanges instances with start(cpg.pair) = C in CpG_1 and end(cpg.pair) = G in CpG_2
watsonMethylCall <- function(x, cpg.pair){
    y1 <- substr(x@elementMetadata$XM, start(cpg.pair) - start(x) + 1, start(cpg.pair) - start(x) + 1)
    y2 <- substr(x@elementMetadata$XM, end(cpg.pair) - start(x), end(cpg.pair) - start(x)) 
    return(data.frame(first = y1, second = y2))
}

## TODO: Rewrite to use functions from the stringr package
...
crickMethylCall <- function(x, cpg.pair){
    y1 <- substr(x@elementMetadata$XM, start(cpg.pair) - start(x) + 2, start(cpg.pair) - start(x) + 2)
    y2 <- substr(x@elementMetadata$XM, end(cpg.pair) - start(x) + 1, end(cpg.pair) - start(x) + 1) 
    return(data.frame(first = y1, second = y2))
}

## TODO: Check if this function may be deprecated
# getMethylCallOld internally finds reads overlapping the CpG pair (this may be more efficiently done outside the function)
# Assumes reads align to + or - strand, i.e. strand=="*" is not allowed
# Returns the results as a data.frame
# Applying table() to the output gives inconsistent row/column orderings, which isn't ideal.
# x = a GRanges instance containing the reads as returned by readBismarkBam()
# cpg.pair = An IRanges instances with start(cpg.pair) = C in CpG_1 and end(cpg.pair) = G in CpG_2
getMethylCallOld <- function(reads, cpg.pair){
  x <- subsetByOverlaps(reads, cpg.pair)
  x <- split(x, strand(x), drop = TRUE)
...
# A list-ified version of getMethylCallOld(). Takes a list of overlaps rather than computing the overlaps internally.
# name = 
# overlaps.list = list of 
# reads = 
# cpg.pairs = 
getMethylCall <- function(name, overlaps.list, reads, cpg.pairs){
  ...
  output <- cbind(output, rep(name, nrow(output)))
  colnames(output)[3] <- "ID"
  return(output)
}


# A function for selecting the CpG pairs for each read (either "leftmost" or random selection)
# reads = a GRanges instance of processed reads as returned by readBismarkBam()
# cpg.pairs = an IRanges instance of CpG pairs as returned by makeCpGPairs()
# select = "first" or "random" - how to select a single CpG pair the read overlaps
# verbose = TRUE or FALSE - only used when select = "random" to produce verbose output for debugging.
# Returns a findOverlaps-style matrix of query-hit pairs, but with each query present only once.
chooseCpGPair <- function(reads, cpg.pairs, select, verbose = FALSE){
  ## Using findOverlaps to do the subsetting (only required to be run once per sample!)
  # A function that for each read chooses the first or "leftmost" CpG pair it overlaps
  ...
  # A function that for each read chooses at random a CpG pair that it overlaps
  randomCpGPair <- function(reads, cpg.pairs, verbose){
    overlaps <- findOverlaps(query = reads, subject = cpg.pairs)
    ...
    # which.pair relates x back to olaps.table (as a row-index)
  ...
  if(select == "first"){
    ...
    randomCpGPair(reads, cpg.pairs, verbose = verbose)
  } else{
    print("Error: select argument must be \'first\' or \'random\'")
  }
}

# Converts a "findOverlaps-style" matrix, e.g. output from chooseCpGPair(), to a list
...
# It acts on a chromosome-by-chromosome basic (otherwise elements of a pair may be from different chromosomes!)
# A makeCpGPairs() object stores the C of CpG_1 in the start() and the G of CpG_2 in the end() of the range
# cpgs = a bed file of CpGs imported using rtracklayer::import()
# type = "disjoint" or "overlapping" - how the pairs are constructed
...
coMethTable <- function(reads, cpgs, pair.choice = "random"){
  ...
  # Create input for the makeMethylDataFrame() function
  ...
  #   list = pos_1
  # etc.
  input.for.makeMethylDataFrame <- lapply(overlaps.list, olaps2List)
    
  # chr.names stores the names of all the chromosomes to be processed
  chr.names <- names(input.for.makeMethylDataFrame)
  
  # Function to create methylation tables for a single chromosome
  makeMethylDataFrame <- function(chr, reads, cpg.pairs, input.for.makeMethylDataFrame){ 
    chr.data <- input.for.makeMethylDataFrame[[chr]]
  ...
  output <- lapply(chr.names, makeMethylDataFrame, reads = reads, cpg.pairs = genome.cpg.pairs, input.for.makeMethylDataFrame = input.for.makeMethylDataFrame)
  names(output) <- names(cpg.pairs)
}

## TODO: Add some print statements to report progress of analysis 
# Computes the comethylation tables for a single sample
# reads = output of readBismarkBam()
# cpg.pairs = a list of RangedData instances storing pairs of CpGs as returned by makeCpGPairs(). Each element of the list corresponds to a chromosome.
# pair.choice = For reads that overlap multiple CpG pairs select whether the "leftmost" pair or a "random" pair is chosen
makeCoMethTable <- function(reads, cpg.pairs, pair.choice = "random"){
  ...
  overlaps.list <- lapply(cpg.pairs, chooseCpGPair, reads = reads, select = pair.choice)
  ...
  # Function to create methylation tables for a single chromosome
  # chr.name = the name of the chromosome to be processsed
  # reads = reads inherited from makeCoMethTable()
  # cpg.pairs = cpg.pairs inherited from makeCoMethtable()
  # input.for.makeMethylDataFrame = output of olaps2List() applied to data for every chromosome (see a few lines above)
  makeMethylDataFrame <- function(chr.name, reads, cpg.pairs, input.for.makeMethylDataFrame){ 
    # Subset the reads-CpGpair overlaps to only those on the chromosome of interest
    chr.data <- input.for.makeMethylDataFrame[[chr.name]]
    # Subset the cpg.pairs to include only those on the chromosome of interest
    cpg.pairs <- cpg.pairs[[chr.name]]
    mclapply(X = names(chr.data), FUN = getMethylCall, overlaps.list = chr.data, reads = reads, cpg.pairs = cpg.pairs)
  }
  
  # Create the tables for every CpG pair on every chromosome
  tab <- lapply(chr.names, makeMethylDataFrame, reads = reads, cpg.pairs = cpg.pairs, input.for.makeMethylDataFrame = input.for.makeMethylDataFrame)
  ...
  # x = list of comethylation tables
  getID <- function(x){
    #unlist(lapply(x, function(x){as.numeric(names(x)[1])}))
    #unlist(lapply(x, function(x){as.numeric(rownames(x)[1])}))
    unlist(lapply(x, function(x){as.numeric(as.character(x$ID[1]))})) # CLUNKY - Perhaps store ID as a numeric to begin with
  }
...
# Should each CpG only contribute to one CpG pair? Yes (for now). In general may not be necessary - see sandpit_tabulate_comethylation.r
# How do these two restrictions affect one another
