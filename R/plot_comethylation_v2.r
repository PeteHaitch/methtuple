...
#### Create data.frame to be used in plots of aggregate log odds ratios stratified by CGI status and average methylation in a 500bp window using complete.wf.all.gr ####
...
complete.wf.all.lor.df$gamma.500.level <- c(rep('Lowly methylated', nrow(lor1)), rep('Intermediately methylated', nrow(lor2)), rep('Highly methylated', nrow(lor3)), rep('Lowly methylated', nrow(lor4)), rep('Intermediately methylated', nrow(lor5)), rep('Highly methylated', nrow(lor6)), rep('Lowly methylated', nrow(lor7)), rep('Intermediately methylated', nrow(lor8)), rep('Highly methylated', nrow(lor9)))

#### Create data.frame to be used in plots of aggregate log odds ratios stratified by CGI status and average methylation in a 500bp window using complete.wf.outermost.gr ####
# Add CGI variable with 3 levels (In, Partially in, Out) to each CpG-pair
...
partially.in <- as.numeric(fully.in == 0 & fully.out == 0)
elementMetadata(complete.wf.outermost.gr)$CGI <- ifelse(fully.in, 'In', ifelse(fully.out, 'Out', 'Partially in'))
# Discretise gamma.500 by tertiles
...
lor4 <- aggregatedLor(subset(complete.wf.outermost.gr, elementMetadata(complete.wf.outermost.gr)$CGI == 'Partially in' & elementMetadata(complete.wf.outermost.gr)$gamma.500.level == 'Low'), ipd = TRUE, nic = FALSE, correct = FALSE)
lor5 <- aggregatedLor(subset(complete.wf.outermost.gr, elementMetadata(complete.wf.outermost.gr)$CGI == 'Partially in' & elementMetadata(complete.wf.outermost.gr)$gamma.500.level == 'Intermediate'), ipd = TRUE, nic = FALSE, correct = FALSE)
lor6 <- aggregatedLor(subset(complete.wf.outermost.gr, elementMetadata(complete.wf.outermost.gr)$CGI == 'Partially in' & elementMetadata(complete.wf.outermost.gr)$gamma.500.level == 'High'), ipd = TRUE, nic = FALSE, correct = FALSE)
...
complete.wf.outermost.lor.df$CGI <- c(rep('Within CGI', nrow(lor1) + nrow(lor2) + nrow(lor3)), rep('Partially in CGI', nrow(lor4) + nrow(lor5) + nrow(lor6)), rep('Outside CGI', nrow(lor7) + nrow(lor8) + nrow(lor9)))
complete.wf.outermost.lor.df$gamma.500.level <- c(rep('Lowly methylated', nrow(lor1)), rep('Intermediately methylated', nrow(lor2)), rep('Highly methylated', nrow(lor3)), rep('Lowly methylated', nrow(lor4)), rep('Intermediately methylated', nrow(lor5)), rep('Highly methylated', nrow(lor6)), rep('Lowly methylated', nrow(lor7)), rep('Intermediately methylated', nrow(lor8)), rep('Highly methylated', nrow(lor9)))

#### Create data.frame to be used in plots of aggregate log odds ratios stratified by CGI status and average methylation in a 500bp window using complete.wf.zero.nic.all.gr ####
# Add CGI variable with 3 levels (In, Partially in, Out) to each CpG-pair
...
partially.in <- as.numeric(fully.in == 0 & fully.out == 0)
elementMetadata(complete.wf.zero.nic.all.gr)$CGI <- ifelse(fully.in, 'In', ifelse(fully.out, 'Out', 'Partially in'))
# Discretise gamma.500 by tertiles
...
lor4 <- aggregatedLor(subset(complete.wf.zero.nic.all.gr, elementMetadata(complete.wf.zero.nic.all.gr)$CGI == 'Partially in' & elementMetadata(complete.wf.zero.nic.all.gr)$gamma.500.level == 'Low'), ipd = TRUE, nic = FALSE, correct = FALSE)
lor5 <- aggregatedLor(subset(complete.wf.zero.nic.all.gr, elementMetadata(complete.wf.zero.nic.all.gr)$CGI == 'Partially in' & elementMetadata(complete.wf.zero.nic.all.gr)$gamma.500.level == 'Intermediate'), ipd = TRUE, nic = FALSE, correct = FALSE)
lor6 <- aggregatedLor(subset(complete.wf.zero.nic.all.gr, elementMetadata(complete.wf.zero.nic.all.gr)$CGI == 'Partially in' & elementMetadata(complete.wf.zero.nic.all.gr)$gamma.500.level == 'High'), ipd = TRUE, nic = FALSE, correct = FALSE)
...
complete.wf.zero.nic.all.lor.df$CGI <- c(rep('Within CGI', nrow(lor1) + nrow(lor2) + nrow(lor3)), rep('Partially in CGI', nrow(lor4) + nrow(lor5) + nrow(lor6)), rep('Outside CGI', nrow(lor7) + nrow(lor8) + nrow(lor9)))
complete.wf.zero.nic.all.lor.df$gamma.500.level <- c(rep('Lowly methylated', nrow(lor1)), rep('Intermediately methylated', nrow(lor2)), rep('Highly methylated', nrow(lor3)), rep('Lowly methylated', nrow(lor4)), rep('Intermediately methylated', nrow(lor5)), rep('Highly methylated', nrow(lor6)), rep('Lowly methylated', nrow(lor7)), rep('Intermediately methylated', nrow(lor8)), rep('Highly methylated', nrow(lor9)))

#### Create data.frame to be used in plots of aggregate log odds ratios stratified by CGI status and average methylation in a 500bp window using complete.wf.zero.nic.outermost.gr ####
# Add CGI variable with 3 levels (In, Partially in, Out) to each CpG-pair
...
partially.in <- as.numeric(fully.in == 0 & fully.out == 0)
elementMetadata(complete.wf.zero.nic.outermost.gr)$CGI <- ifelse(fully.in, 'In', ifelse(fully.out, 'Out', 'Partially in'))
# Discretise gamma.500 by tertiles
...
lor4 <- aggregatedLor(subset(complete.wf.zero.nic.outermost.gr, elementMetadata(complete.wf.zero.nic.outermost.gr)$CGI == 'Partially in' & elementMetadata(complete.wf.zero.nic.outermost.gr)$gamma.500.level == 'Low'), ipd = TRUE, nic = FALSE, correct = FALSE)
lor5 <- aggregatedLor(subset(complete.wf.zero.nic.outermost.gr, elementMetadata(complete.wf.zero.nic.outermost.gr)$CGI == 'Partially in' & elementMetadata(complete.wf.zero.nic.outermost.gr)$gamma.500.level == 'Intermediate'), ipd = TRUE, nic = FALSE, correct = FALSE)
lor6 <- aggregatedLor(subset(complete.wf.zero.nic.outermost.gr, elementMetadata(complete.wf.zero.nic.outermost.gr)$CGI == 'Partially in' & elementMetadata(complete.wf.zero.nic.outermost.gr)$gamma.500.level == 'High'), ipd = TRUE, nic = FALSE, correct = FALSE)
...
complete.wf.zero.nic.outermost.lor.df$CGI <- c(rep('Within CGI', nrow(lor1) + nrow(lor2) + nrow(lor3)), rep('Partially in CGI', nrow(lor4) + nrow(lor5) + nrow(lor6)), rep('Outside CGI', nrow(lor7) + nrow(lor8) + nrow(lor9)))
complete.wf.zero.nic.outermost.lor.df$gamma.500.level <- c(rep('Lowly methylated', nrow(lor1)), rep('Intermediately methylated', nrow(lor2)), rep('Highly methylated', nrow(lor3)), rep('Lowly methylated', nrow(lor4)), rep('Intermediately methylated', nrow(lor5)), rep('Highly methylated', nrow(lor6)), rep('Lowly methylated', nrow(lor7)), rep('Intermediately methylated', nrow(lor8)), rep('Highly methylated', nrow(lor9)))
