...
#### WARNING ####
# comethylation_by_methylation_level.r is SUPERSEDED by plot_comethylation_by_methylation_level.r
...
# If an object is used in multiple findOverlaps() calls, e.g. AM.gr, create an IntervalTree version of it.
# Compare cross-classification by n_w (the number of CpGs in the window of size w-bp) with cross-classification by in/out of CGI
...
# 'x' is a GRanges object, e.g. 'merged', 'w' is the window size, 'AM.gr' is the AM.gr object that contains the beta-values for all CpGs in the sample
# 'x' must have well-defined 'seqlengths' attribute. Otherwise 'resize' function cannot correctly trim regions with negative start values or end values > length of the chromosome
# Code based on https://stat.ethz.ch/pipermail/bioc-sig-sequencing/2010-June/001325.html
averageMethylationInWindow <- function(merged, AM.gr, w){
  regions <- resize(merged, fix = 'center', width = w)
  ol <- findOverlaps(regions, AM.gr)
  ...
  means <- data.frame(beta = viewMeans(v.beta), gamma = viewMeans(v.gamma))
  return(means)
}
...
CpG.pairs <- GRanges(seqnames = seqnames(WF.gr), ranges = ranges(WF.gr), lor = lors, coverage = coverage, n_500 = n_500, beta_500 = average.methylation_500$beta, gamma_500 = average.methylation_500$gamma, CGI = in.CGI)
...
pdf(file = paste(sample.name, '_CpG-density_500_ECDF.pdf', sep = ''))
