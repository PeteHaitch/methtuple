...
def extract_and_update_methylation_index_from_single_end_read(read, BAM, methylation_m_tuples, m, methylation_type, methylation_pattern, ignore_start_r1, ignore_end_r1, min_qual, phred_offset, ob_strand_offset):
    ...
    Returns:
        ...
        m_methylation_loci: The number of methylation loci extracted from the read.
    """
    ...
    m_methylation_loci = len(methylation_index)
    # Case A: >= m methylation loci in the read
    if m_methylation_loci >= m:
        ...
        # Else, construct each bookended methylation-loci m-tuple and add it to the methylation_m_tuple instance.
            ...
            for i in range(0, len(methylation_index) - m + 1): # For a read containing k methylation loci there are (k - m + 1) m-tuples.
                ...
                if not m_tuple_id in methylation_m_tuples:
                    ...
                    methylation_m_tuples[m_tuple_id].increment_count(''.join([read.opt('XM')[j] for j in methylation_index[i:(i + m)]]), methylation_type, read, None)
    return methylation_m_tuples, m_methylation_loci
...
def extract_and_update_methylation_index_from_paired_end_reads(read_1, read_2, BAM, methylation_m_tuples, m, methylation_type, methylation_pattern, ignore_start_r1, ignore_start_r2, ignore_end_r1, ignore_end_r2, min_qual, phred_offset, ob_strand_offset, overlap_check, n_fragment_skipped_due_to_bad_overlap):
    ...
    if n_overlap > 0:
        if is_overlapping_sequence_identical(read_1, read_2, n_overlap, overlap_check):
            ...
            methylation_index_1 = []
            methylation_index_2 = []
    m_methylation_loci = len(methylation_index_1) + len(methylation_index_2)
...
for read in BAM:
    ...
    # Read is second in a read-pair
        ...
        if read_1.tid == read_2.tid and read_1.qname == read_2.qname:
            methylation_m_tuples, m_methylation_loci_in_fragment, n_fragment_skipped_due_to_bad_overlap = extract_and_update_methylation_index_from_paired_end_reads(read_1, read_2, BAM, methylation_m_tuples, m, methylation_type, methylation_pattern, ignore_start_r1, ignore_start_r2, ignore_end_r1, ignore_end_r2, min_qual, phred_offset, ob_strand_offset, overlap_check, n_fragment_skipped_due_to_bad_overlap)
            # Update the n_methylation_loci_per_read dictionary
            if not m_methylation_loci_in_fragment in m_methylation_loci_per_read:
                m_methylation_loci_per_read[m_methylation_loci_in_fragment] = 0
            m_methylation_loci_per_read[m_methylation_loci_in_fragment] += 1
    ...
    # Read is single-end
        ...
        if does_read_contain_indel(read):
            continue
        methylation_m_tuples, m_methylation_loci_in_fragment = extract_and_update_methylation_index_from_single_end_read(read, BAM, methylation_m_tuples, m, methylation_type, methylation_pattern, ignore_start_r1, ignore_end_r1, min_qual, phred_offset, ob_strand_offset)
        if not m_methylation_loci_in_fragment in m_methylation_loci_per_read:
            m_methylation_loci_per_read[m_methylation_loci_in_fragment] = 0
        m_methylation_loci_per_read[m_methylation_loci_in_fragment] += 1
...
for k, v in m_methylation_loci_per_read.iteritems():
    if k >= m:
...
print 'n\tcount'
for k, v in iter(sorted(m_methylation_loci_per_read.iteritems())):
    print k, '\t', v
